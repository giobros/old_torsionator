...
class Workflow:
    ...
    def _detect_clashes(
        ...
        for method in methods:
            ...
            for d_tup in dihedrals_to_scan:
                ...
                has_clash = not getattr(res, "no_clash", True)
    ...
    def _handle_conformers(
        ...
        if all(not clash_by_method[m] for m in methods):
        ...
        for m in methods:
            ...
            for d_tup in sorted(clash_by_method[m]):
                ...
                if bcs_norm == "true" and res.get("best_lj"):
                    best_conf[m][d_tup] = res["best_lj"]["path"]
                    self.log.warning(
                        "[CONF][%s][%s] no clash-free found; BCS=true -> best_lj: %s (LJ=%.4f)",
                        m, d_tup, best_conf[m][d_tup], res["best_lj"].get("lj_value", float("nan")),
                    )
                    if res.get("best_rg"):
                        self.log.info(
                            "[CONF][%s][%s] best_rg: %s (RG=%.4f)",
                            m, d_tup, res["best_rg"].get("path"), res["best_rg"].get("rg_value", float("nan")),
                        )
                    continue
        ...
        for m in methods:
            ...
            for d_tup in dihedrals_to_scan:
                ...
                io_utils.run_gaff2_old(
                    ...
                    basis_periodicities=basis_tuple,
                    postscan_make_expanded_for_mdgx=expand_basis,
                    MCS=False, mcs_xyz=None, step_size=step_size,
                )

                param_results = io_utils.run_parametrization(
                    self.log, self.cfg, m, d_tup,
                    MCS=False, basis_periodicities=basis_tuple, expand_basis=expand_basis,
                )
                ...
                try:
                    plot_shifted_energy_profiles(self.cfg.base_dir, d_tup, best_profile)
        ...
        *,
        ...
        for m in methods:
            ...
            for d_tup in dihedrals:
                ...
                io_utils.run_gaff2_old(
                    self.log, self.cfg, root_name, m, d_tup, pdb_file,
                ...
                param_results = io_utils.run_parametrization(
                    self.log, self.cfg, m, d_tup,
                    MCS=True, basis_periodicities=basis_tuple, expand_basis=expand_basis,
                )
                ...
                try:
                    plot_shifted_energy_profiles_mcs(
                        self.cfg.base_dir, d_tup, methods=(m,), best_profile=best_profile,
                    )
                except Exception as exc:
