...
def extract_eamber(out_file: str) -> float:
    """Parse EAMBER from a sander minimization output (non-MCS path)."""
    last = None
    with open(out_file) as f:
        for line in f:
            if "EAMBER" in line:
                last = line
    if last is None:
        raise RuntimeError(f"No EAMBER line found in {out_file}")
    m = re.search(r"EAMBER\s*=\s*([-\d\.Ee+]+)", last)
    if not m:
        raise RuntimeError(f"Could not parse EAMBER from: {last.strip()}")
...
def _canon_per(per: float) -> int:
    ...
    if abs(p) == 1:
        return 1
    return -abs(p) if abs(p) > 1 else p
...
def find_dihedral_key_in_frcmod(
    ...
    for k in candidates:
        if k in key_to_indices:
            if log:
                log.info("[FIND-DIHE] matched key=%r", k)
            return k
    for k in candidates:
        rk = _reverse_key(k)
        if rk in key_to_indices:
            if log:
                log.info("[FIND-DIHE] matched reversed key=%r (from %r)", rk, k)
            return rk
    if log:
        log.warning(
...
def param_input_file(
    ...
    for path, label in [(energy_file, "energies.dat"), (traj, "geometries.nc"), (frcmod, "frcmod"), (leap_top, "MOL.top")]:
...
class Gaff2SingleDihedralRefinement(_AmberRefinementBase):
    ...
    def _resolve_angles(self, input_pdb, d_tup, step_size, start_from_nearest):
        if start_from_nearest:
            try:
                ...
                self.log.warning("[GAFF2/old] start_from_nearest failed (%s); fallback to 0", e)
...
class Gaff2NewDihedralRefinement(_AmberRefinementBase):
    ...
    def refine_new(
        ...
        for hrst_val, paths in sorted(param_results.items()):
            ...
            if os.path.abspath(mol2_src) != os.path.abspath(mol2_dst):
                shutil.copy2(mol2_src, mol2_dst)
            if os.path.abspath(frcmod_src) != os.path.abspath(frcmod_dst):
    ...
    def _resolve_angles(self, input_pdb, d_tup, step_size, start_from_nearest):
        if start_from_nearest:
            try:
                ...
                self.log.warning("[GAFF2/new] start_from_nearest failed (%s); fallback to 0", e)
        return angles_0_350(step_size)
