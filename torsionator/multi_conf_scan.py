...
def _load_angles_energies(path_txt: str):
    ...
    if data.ndim == 1:
        if data.size == 2:
            return data.reshape(1, 2)[:, 0], data.reshape(1, 2)[:, 1]
...
def _MCS_merge_scans(
    ...
    # best[bin] = (E_compare, xyz_path, ang_true, tag, E_abs)
    best: Dict[int, Tuple] = {b: None for b in angle_bins}
    ...
    for tag in scanned_tags:
        ...
        for ang, E_cmp, E_abs in zip(angles_deg, energies_compare, energies_eh):
            ...
            cur = best[bin_angle]
            if cur is None or E_cmp < cur[0] - 1e-15:
                best[bin_angle] = (E_cmp, xyz_path, ang_norm, tag, E_abs)
    ...
    for b, (E_cmp, xyz_path, ang_true, tag, E_abs) in chosen:
        frame = read(xyz_path)
        header = f"angle={ang_true:.2f} E={E_abs:.8f} conformer={tag}"
        write_xyz_file(xyz_out, frame, mode="a", sig=16, header=header)
        energies_abs.append(E_abs)
