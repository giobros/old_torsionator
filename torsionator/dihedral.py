...
def _shift_scan_files(output_folder: str) -> None:
    ...
    df = pd.read_csv(raw, delim_whitespace=True, header=None, names=["Angle", "Energy"])
...
class GraphSplitter:
    ...
    def split_by_dihedral(
        ...
        *,
        ...
        assert not (set(group_A) & set(group_D))
        assert set(group_A) | set(group_D) == set(range(len(atoms)))
        return group_A, group_D

    @staticmethod
    def _build_connectivity_graph(atoms):
            ...
            for i in AN:
                for l in BN:
                    if (
                        ...
                        and mol.GetAtomWithIdx(l).GetAtomicNum() > 1
                    ):
...
def plot_shifted_energy_profiles(
    ...
    """Plot standard (non-MCS) energy profiles: NN/ML, GAFF2-old, GAFF2-best."""
    ...
    for label in ("obi", "mace"):
        qm_path = os.path.join(base, label, "angles_vs_energies_final.txt")
        if os.path.isfile(qm_path):
            data = np.loadtxt(qm_path)
            plt.plot(data[:, 0], data[:, 1] * CONV_EH_TO_KCAL_MOL, "o-", label=f"{label} QM")
            has_data = True

        gaff2_old_path = os.path.join(base, "GAFF2", "old", label, "dd_ee_shifted")
        if os.path.isfile(gaff2_old_path):
            data = np.loadtxt(gaff2_old_path)
            plt.plot(data[:, 0], data[:, 1], "s--", label=f"{label} GAFF2")
            has_data = True

    if best_profile and os.path.isfile(best_profile):
        data = np.loadtxt(best_profile)
        plt.plot(data[:, 0], data[:, 1], "^-.", label="GAFF2_best")
        has_data = True
    ...
    if has_data:
        ...
        plt.title(f"Dihedral Scan Energy Profile: {i}-{j}-{k}-{l}")
        plt.legend()
        plt.xlim(0, 360)
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(os.path.join(base, f"{dih_str}.png"))
        logging.info("Saved plot: %s", os.path.join(base, f"{dih_str}.png"))
    plt.close()
...
def plot_shifted_energy_profiles_mcs(
    base_dir: str,
    dihedral: Tuple[int, int, int, int],
    methods: Tuple[str, ...] = ("obi", "mace"),
    best_profile: Optional[str] = None,
) -> None:
    """Plot MCS energy profiles: NN/ML-MCS, GAFF2-old-MCS, GAFF2-best."""
    import matplotlib.pyplot as plt
    ...
    for label in methods:
        mcs_path = os.path.join(base, label, "MCS", "angles_vs_energies_final.txt")
        if os.path.isfile(mcs_path):
            data = np.loadtxt(mcs_path)
            plt.plot(data[:, 0], data[:, 1] * CONV_EH_TO_KCAL_MOL, "o-", label=f"{label}-MCS")
            has_data = True

        gaff2_old_path = os.path.join(base, "GAFF2", "old", label, "dd_ee_shifted")
        if os.path.isfile(gaff2_old_path):
            data = np.loadtxt(gaff2_old_path)
            plt.plot(data[:, 0], data[:, 1], "s--", label=f"{label} GAFF2_MCS")
            has_data = True
    ...
    if best_profile and os.path.isfile(best_profile):
        data = np.loadtxt(best_profile)
        plt.plot(data[:, 0], data[:, 1], "^-.", label="GAFF2_rep_best_MCS")
        has_data = True
    ...
    if has_data:
        ...
        plt.xlabel("Dihedral Angle (degrees)")
        plt.ylabel("Relative Energy (kcal/mol)")
        plt.title(f"MCS Dihedral Scan Energy Profile: {i}-{j}-{k}-{l}")
        plt.legend()
        plt.xlim(0, 360)
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(os.path.join(base, f"{dih_str}_MCS.png"))
        logging.info("Saved MCS plot: %s", os.path.join(base, f"{dih_str}_MCS.png"))
    plt.close()
